...
void Target::Publishing()
	{
		...
		if (ms_rtiAmb)
		{
			...
			try
			{
				...
				//TargetAttributes->add(m_AccelerationHandle);
...
void Target::SetUpdateControl(RTI::Boolean status, const RTI::AttributeHandleSet& theAttrHandles)
{
	...
	for (unsigned int i = 0; i < theAttrHandles.size(); i++)
	{
		...
		if (attrHandle == Target::GetPositionRtiId())
		{
			...
			cout << "FED_HW: Turning Target. Position Updates "
		...
		else if (attrHandle == Target::GetVelocityRtiId())
		{
			...
			cout << "FED_HW: Turning Target. Velocity Updates "
				<< pStr << " for object " << this->GetInstanceId()
				<< ". " << endl;
	...
	if ((myTarget)&&(ep))
	{
...

		{
		...
		else if (mode == 3)
			{
				...
				cout << "****Please type the acceleration of Target:" << myTarget->GetName() << endl;
				for (int i = 0; i < 3; i++)
				{
					cin >> tgta[i];
				}
...

{
	...
	engEvalString(ep, " env2  = setTarget( fs,fc,mode,rcs,tgt_Az,tgt_el,tgt_r,vx,vy,vz,tgt_ax,tgt_ay,tgt_az );");
