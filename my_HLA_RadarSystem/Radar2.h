...
public:
	Radar2();
	Radar2(RTI::ObjectHandle id);
	virtual ~Radar2();
	//-------------------------
	// Methods acting on the RTI
	//--------------------------
	static Radar2*   Find(RTI::ObjectHandle objectId);
	static void		Init(RTI::RTIambassador* rtiAmb);
	void			Publishing();
	void			Subscribing();
	void			Register();
	void			RadarUpdate(Radar2* pRadar);
	//void			ProcessOwnership(Radar2* myRadar, RTI::Boolean defector, int loop);
	void			DeleteInstance(Radar2* pRadar);
	//--------------------------
	// Matlab methods
	//--------------------------

	void setMATLABRadar(Radar2* myRadar, Engine* ep, int posIdx);
	void getMATLABEcho(Radar2* myRadar, Engine* ep, int Idx);
	void getMATLABRadarStatus(Radar2* myRadar, Engine* ep, int Idx);
	void getMATLABdetection(Radar2* myRadar, Engine* ep, int Idx);
	void Radar2::generateBeamPosition(Engine* ep);

	//-----------------------------------------------------------------
	// operator <<
	//-----------------------------------------------------------------
	friend ostream& operator << (ostream &s, Radar2 &v);
	friend ostream& operator << (ostream &s, Radar2 *v);
	...
	// static members
	//-----------------------------
	static Radar2*		ms_RadarExtent[4];
